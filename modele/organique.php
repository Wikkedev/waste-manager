...
if ($sortingCenter->getCapacite() >= $totalOrganic)
{
    ...
    if ($capaciteComposting > $totalOrganic)
    {
        ...
        echo "Le compostage des dechets organiques a produit ".$Composter->compostingTreatment($organicCompostCo2, $totalOrganic)." grammes de CO2.<br>".PHP_EOL;
        //array_push($co2RejeteCompost, $Composter->OrganicTreatment($organicCompostCo2, $totalOrganic));
        $co2OrganicTotal += $composter->compostingTreatment($organicCompostCo2, $totalOrganic);
    }
    else
    {
        //je recupere ce qu'il reste à traiter que j'envoie à l'incinerateur
        $resteOrganic = $totalOrganic - $capaciteComposting;
